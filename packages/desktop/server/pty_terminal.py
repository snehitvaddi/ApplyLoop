...
class PTYSession:
    ...
    def _build_initial_prompt(cwd: str, snapshot: dict) -> str:
        ...
        lines.append("  If the user asks you to change the exclusion list, preferences, profile,")
        lines.append("  or any setting — DO IT via curl to the cloud API, then update local files.")
        lines.append("  Example: 'Add Microsoft to exclusion list' →")
        lines.append(f"    curl -sS -X PUT '{env.get('NEXT_PUBLIC_APP_URL', 'https://applyloop.vercel.app')}/api/settings/preferences' \\")
        lines.append(f"      -H 'X-Worker-Token: {worker_token_hint}' -H 'Content-Type: application/json' \\")
        lines.append("      -d '{\"excluded_companies\": [\"Microsoft\", ...existing...]}'")
        lines.append("  Then update ./profile.json locally to match. No asking. Just do it.")
        lines.append("")
